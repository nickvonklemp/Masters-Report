...
%====================================================
The various allocation rules, as described in Ch:\ref{ch:allocation}, implement virtual control inputs to solve for explicit actuator positions. The abstraction applied to achieve an affine relationship required for inversion allocation, below in Eq:\ref{}, meant that actuator transfer rates were independent from the allocation rule applied.
...
\end{equation}
\end{subequations}
A pseudo inversion allocation solves for $\vec{F}$ using $B^\ddagger(\vec{\mathbf{x}},t)\vec{\nu}_d$, from Eq:\ref{eq:pseudo-inversion}. A combined attitude \emph{and} position step, Eq:\ref{eq:attitude-step-position} and Eq:\ref{eq:position-step} setpoints respectively, in Fig:\ref{} shows the reference case to be improved on. Recalling the combined setpoint:
\begin{equation}
