...
\end{equation}
\end{subequations}
Instead, returning to rotation matrices for the transformation and reiterating that Euler angle equivalents for the servos are; $[\phi,~\theta,~\psi]^T\Rightarrow [\lambda_i,~\alpha_i,~\sigma_i]^T$. It then follows (\emph{with $i^{th}$ motor subscripts implied}):
\begin{subequations}
\begin{equation}
