...
\end{equation}
\end{subequations}
However, that stabilizing virtual plant input $\Omega_d$ has its own associated error, $z_1$, which needs to be stabilized:
\begin{subequations}
\begin{equation}
...
\end{equation}
\end{subequations}
Introducing that error $z_1$ into a second LCF, which expands the first proposed LCF:
\begin{subequations}
\begin{equation}
...
\end{equation}
With $\Gamma_2$ being another positive definite symmetric coefficient matrix. Then with the control law $\mu\vec{\tau}_{_{IBC}}$ introduced the LCF derivative $\dot{V}_2$ reduces to negative definite:
\begin{equation}
