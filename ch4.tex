...
V_{_{XPD}}\big(\vec{q}_e,~\widetilde{\Omega}\big)=\vec{q}_e\text{}^T\vec{q}_e+\big(1-q_0\big)^2+\frac{1}{2}\widetilde{\Omega}\text{}^{\hspace{1pt}T}\Big(\Gamma_3^{-1}J_b(u)\Big)\widetilde{\Omega}
\end{equation}
...
V_1(\vec{q}_e)=\vec{q}_e\text{}^T\vec{q}_e+(1-q_0)^2
\end{equation}
...
\vspace{-6pt}
\begin{equation}
=\vec{q}_e\text{}^T\vec{q}_e+(1-q_0)^2+\frac{1}{2}z_1\text{}^Tz_1
\end{equation}
\end{subequations}
...
\vspace{-10pt}
\begin{equation}
=-\vec{q}_e\text{}^T\Gamma_1\vec{q}_e-z_1\text{}^T\Gamma_2z_1< 0~~~~\forall~(\vec{q}_e,z_1)
\end{equation}
\end{subequations}
...
\end{equation}
\end{subequations}
Whilst stabilizing, the IBC controller is not globaly stable. The plant conditions require exact plant matching and account for no unmodelled disturbances or measurement uncertainty. In practice the introduction of some disturbance torque $\vec{L}$ could easily drive Eq:\ref{eq:ibc-deriv} away from negative-definite such that stability is lost.
%====================================================
\subsubsection{Adaptive Backstepping Controller}
\label{subsubsec:control.attitude.nonlinear.adaptivebackstep}
%====================================================
Considering only a lumped uncertainty/disturbance term for the adaptive case and assuming state dependent uncertainty can be included in such a term. Adding that term, a lumped $\vec{L}$, into the angular acceleration dynamics:
\begin{equation}
\dot{\vec{\omega}}_b=J_b\text{}^{-1}(u)\big(-\vec{\omega}_b\times J_b(u)\vec{\omega}_b-\hat{\tau}_b(u)+\vec{\tau}_g+\vec{\tau}_Q+\vec{L}+\vec{\tau}_{_{ABC}}\big)
\end{equation}
An unmoderated disturbance acts as an external torque on Eq:\ref{eq:newton}. Obviously estimation errors and disturbances could simply be compensated for in the control law; $-\vec{L}$. However, it is difficult to approximate disturbances without any \emph{apriori} knowledge about of its properties. Noise compensation in sensors can be done easily due to the known frequency bandwidth which that noise occurs in, the same cannot be said for wind disturbances or payload variations\ldots
\par
An approximate disturbance estimation, $\hat{L}$, is used for that compensation in the designed control torque $\vec{\tau}_{_{ABC}}$. Each estimate will have its own error deviation from a true $\vec{L}$ acting on the body:
...
\par
The estimate error is introduced to an LFC extended from the IBC case in Eq:\ref{eq:ibc-lfc}:
\begin{subequations}
\begin{equation}
V_{_{ABC}}(\vec{q}_e,z_1,\Delta L)=V_{_{IBC}}(\vec{q}_e,z_1)+\frac{1}{2}\Delta L^T \Gamma_L^{-1}\Delta L
\end{equation}
\vspace{-10pt}
\begin{equation}
=\vec{q}_e\text{}^T\vec{q}_e+(1-q_0)^2+\frac{1}{2}z_1\text{}^Tz_1+\frac{1}{2}\Delta L^T\Gamma_L^{-1}\Delta L
\end{equation}
\end{subequations}
Where the $3\times 3$ positive symmetric coefficient matrix $\Gamma_L$ is an adaptation gain matrix. Those particular coefficients determine rate at which the system \emph{adapts} to disturbances. To prove stability one starts with the LFC derivative $\dot{V}_{_{ABC}}$:
\begin{equation}
\dot{V}_{_{ABC}}(\vec{q}_e,z_1,\Delta L)=\dot{V}_{_{IBC}}(\vec{q}_e,~z_1)+\frac{1}{2}\Delta\dot{L}^T\Gamma_L^{-1}\Delta L+\frac{1}{2}\Delta L^T\Gamma_L^{-1}\Delta\dot{L}
\end{equation}
Recalling the definition of $\Delta L$ from Eq:\ref{eq:estimate-error}. For its derivative $\Delta \dot{L}$ it is reasonable to assume the dynamics of the physical disturbance $\vec{L}$ are far slower than that of the control system, or that $\dot{\vec{L}}<<\dot{\hat{L}}$. Then it follows:
\begin{equation}
\Delta\dot{L}=\dot{\vec{L}}-\dot{\hat{L}}\approx\vec{0}-\dot{\hat{L}}=-\dot{\hat{L}}
\end{equation}
...
\\
-J_b^{-1}(u)\big(-\vec{\omega}_b\times J_b(u)\vec{\omega}_b-\hat{\tau}_b(u)+\vec{\tau}_g+\vec{\tau}_Q+\vec{L}+\vec{\tau}_{_{ABC}}\big)\bigg)-\Delta L^T\Gamma_L^{-1}\dot{\hat{L}}
\end{multline}
Note that the physical disturbance term, $\vec{L}$, is included in  Eq:\ref{eq:abc-lfc-deriv}. Extending the ideal back stepping control law, $\vec{\tau}_{_{IBC}}$ from Eq:\ref{eq:control-ibc}, to include a disturbance estimate term:
\begin{multline}
...
\end{multline}
Torque disturbances, $\vec{L}$, act in the body frame and require no reference frame transformation. Further note that the control law compensates for disturbances with an estimate term. The energy function's derivative, $\dot{V}_{_{ABC}}$, then reduces to:
\begin{equation}
\dot{V}_{_{ABC}}=\dot{V}_{_{IBC}}-z_1\text{}^TJ_b^{-1}(u)\Big(\vec{L}-\hat{L}\Big)-\Delta L^T\Gamma_L^{-1}\dot{\hat{L}}
\end{equation}
\vspace{-10pt}
\begin{equation}
=-\vec{q}_e\text{}^T\Gamma_1\vec{q}_e-z_1\text{}^T\Gamma_2z_1-z_1\text{}^TJ_b^{-1}(u)\Delta L-\Delta L^T\Gamma_L^{-1}\dot{\hat{L}}
\end{equation}
\vspace{-6pt}
\begin{equation}
=-\vec{q}_e\text{}^T\Gamma_1\vec{q}_e-z_1\text{}^T\Gamma_2z_1-
\Delta L^T\Gamma_L^{-1}\Big(\dot{\hat{L}}+\Gamma_LJ_b^{-1}(u)z_1\Big)
\end{equation}
\end{subequations}
The decision then needs to be made as to how the disturbance estimate is going to be updated such that asymptotic stability is assured; $\dot{V}_{_{ABC}}<0$. The obvious choice for $\dot{\hat{L}}$ would be to compensate for $\Gamma_LJ_b^{-1}(u)z_1$ in the LFC:
...
\end{equation}
Typical stabilizing limits exists from Eq:\ref{eq:abc-asymptotic} but, most importantly $\lim_{t\rightarrow\infty}\Delta L\rightarrow \vec{0}$. But no further conclusions can be drawn about that stability in Eq:\ref{eq:abc-asymptotic}. A lot of work has been done on the statistical nature of disturbance approximation and how best to adapt a non-linear control system to the influence of unwanted disturbances, \cite{nonlineardisturbance,disturbanceadaptivebackstepping,robusttrackingcontrol}. Alternatively, the previous disturbance estimate $\vec{L}=\hat{L}_{n-1}$ could be used, such that:
\begin{subequations}
\begin{equation}
\Delta L'\triangleq\vec{L}-\hat{L}=(\hat{L}_{n-1}-\hat{L}_n)
\end{equation}
Then proposing a $1^{\text{st}}$ order approximation update rule for $\dot{\hat{L}}$ to replace Eq:\ref{eq:asymptotic-disturbance}:
\begin{equation}
\dot{\hat{L}}=-\Gamma_LJ_b^{-1}z_1+\Delta L'
\end{equation}
\vspace{-12pt}
\begin{equation}
=-\Gamma_L\Gamma_1J_b^{-1}(u)\vec{q}_e+\Gamma_LJ_b^{-1}(u)\vec{\omega}_b+(\hat{L}_{n-1}-\hat{L}_n)
\end{equation}
With that rule substituted into the LFC from Eq:\ref{eq:abc-lfc-deriv} yields:
\begin{equation}
\therefore\dot{V}_{_{ABC}}'=-\vec{q}_e\text{}^T\Gamma_1\vec{q}_e-z_1\text{}^T\Gamma_2z_1-\Delta L\text{}^T\Gamma_L^{-1}\big(\hat{L}_{n-1}-\hat{L}_n\big)
\end{equation}
\vspace{-12pt}
\begin{equation}
=-\vec{q}_e\text{}^T\Gamma_1\vec{q}_e-z_1\text{}^T\Gamma_2z_1-\Delta L\Gamma_L^{-1}\Delta L'
\end{equation}
\end{subequations}
...
\underset{t\rightarrow\infty}{\lim}\Delta L'=\big(\hat{L}_{n-1}-\hat{L}_n\big)\rightarrow\Delta L
\end{equation}
...
\begin{equation}
\dot{V}_{_{ABC}}(\vec{q}_e,z_1,\widetilde{L})=-\vec{q}_e\text{}^T\Gamma_1\vec{q}_e-z_1\text{}^T\Gamma_2z_1-\Delta L^T\Gamma_L^{-1}\Delta L
\end{equation}
...
%====================================================
Only two control laws for position control are presented here, the primary focus is on attitude controllers derived in the antecedent. From the Coriolis cross-coupling attitudes are stabilized independently from the position loop (Eq:\ref{eq:quaternion-states-angular} and Eq:\ref{eq:quaternion-states-acceleration}) the converse is, however, not true. A simple Proportional-Derivative control structure is presented as the reference case. thereafter an adaptive backstepping control algorithm is proposed. The dynamics for position control, Eq:\ref{eq:quaternion-states-acceleration}, include a coupled angular velocity component.
...
\end{equation}
With $\vec{G}_b$ being the gravitational acceleration vector in the body frame. Given the standard operating conditions of a quadrotor (App:\ref{app:equations.standard}) it is assumed that $\vec{\omega}_b\approx\vec{0}$ or that inherent angular velocity coupled dynamics are negligible; $\vec{\omega}_b\times m\vec{v}_b\approx 0$. If the plant model is known or atleast approximated with a relative degree of certainty, it is easy to compensate for those dynamics rather than making assumptions about their influence on the system. That plant dependency can be compensated in the control force $\vec{F}_\mu(u)$. 
\par
The translational velocity, $\vec{v}_b$, defined in the body frame is related to the inertial frame position rates through a quaternion transformation:
\begin{equation}
...
\end{equation}
With $\vec{\mathcal{E}}_d(t)$ being some position setpoint from the trajectory generation block. The translational velocity error can be similarly calculated but, in the same way that the angular velocity setpoint is $\vec{\omega}_d=\vec{0}$, the desired translational velocity is zero.
\begin{equation}
\dot{\vec{\mathcal{E}}}_e=\dot{\vec{\mathcal{E}}}_d-\dot{\vec{\mathcal{E}}}_b=-\dot{\vec{\mathcal{E}}}_b(t)\Big|_{\dot{\vec{\mathcal{E}}}_d=\vec{0}}
\end{equation}
...
%==================================================== 
Starting with a simple PD structure to use for reference. The plant dependent controller designs the net force proportional to both the position error and the first derivative velocity error\begin{subequations}\label{eq:position-pd}
\begin{equation}
...
\end{equation}
\end{subequations}
The stability proof requires that error states are transformed to the body frame $\mathcal{F}^b$, such that the control input and error states all act in a common frame. Defining an error state in the body frame $X_e$:
\begin{subequations}
...
\end{equation}
Recalling the difference between translational rates and velocity in Eq:\ref{eq:quaternion-states-velocity}, translational position rates are then:
...
\begin{subequations}
\begin{equation}
\vec{F}_{PD}=K_p\vec{X}_e + K_d\dot{\vec{X}}_e + \vec{\omega}_b\times m_b\vec{v}_b-m_b\vec{G}_b
\end{equation}
\vspace{-15pt}
\begin{equation}
...
\begin{subequations}
\begin{equation}
V_{_{PD}}(\vec{X}_e,\dot{\vec{X}}_e)=\frac{1}{2}\vec{X}_e\text{}^TK_p\vec{X}_e+\frac{1}{2}\dot{\vec{X}}_e\text{}^Tm\dot{\vec{X}}_e~~>0,~\forall(\vec{X}_e,\dot{\vec{X}}_e)
\end{equation}
\vspace{-12pt}
\begin{equation}
=\frac{1}{2}\vec{X}_e\text{}^TK_p\vec{X}_e+\frac{1}{2}\vec{v}_b\text{}^Tm\vec{v}_b\Big|_{\dot{\vec{X}}_e=-\vec{v}_b}
\end{equation}
\end{subequations}
Then calculating the LFC derivative with the PD control law substituted:
\begin{subequations}
\begin{equation}
\dot{V}_{_{PD}}(\vec{X}_e,\dot{\vec{X}}_e)=K_p \vec{X}_e\text{}^T\dot{\vec{X}}_e+\vec{v}_b\text{}^Tm_b\dot{\vec{v}}_b
\end{equation}
\vspace{-10pt}
\begin{equation}
=-K_p \vec{X}_e\text{}^T\vec{v}_b+\vec{v}_b\text{}^Tm_b\dot{\vec{v}}_b
\end{equation}
\vspace{-8pt}
\begin{equation}
=-K_p \vec{X}_e\text{}^T\vec{v}_b+\vec{v}_b\text{}^T\big(-\vec{\omega}_b\times m_b\vec{v}_b+m_b\vec{G}_b+\vec{F}_{_{PD}}\big)
\end{equation}
\vspace{-8pt}
\begin{equation}
=-K_p \vec{X}_e\text{}^T\vec{v}_b+\vec{v}_b\text{}^T\big(K_p \vec{X}_e-K_d\vec{v}_b\big)
\end{equation}
\vspace{-8pt}
\begin{equation}\label{eq:position-pd-stability}
\therefore\dot{V}_{_{PD}}=-K_d\vec{v}_b\text{}^T\vec{v}_b<0~~~~\forall~(\vec{X}_e,\dot{\vec{X}}_e)
\end{equation}
\end{subequations}
The global stability asserted in Eq:\ref{eq:position-pd-stability} holds for $\forall(\vec{\mathcal{E}}_e,\dot{\vec{\mathcal{E}}}_e)$, irrespective of the transformation applied in Eq:\ref{eq:4.80a} and Eq:\ref{eq:4.80b}. It then follows that the following global asymptotically stabilizing limits exist:
\begin{subequations}
\begin{equation}
...
\vspace{-8pt}
\begin{equation}
\underset{t\rightarrow\infty}{\lim}\dot{X}_e=Q_b^*\otimes(\dot{\vec{\mathcal{E}}}_d-\dot{\vec{\mathcal{E}}}_b)\otimes Q_b=-\vec{v}_b\Big|_{\dot{\vec{\mathcal{E}}}_e=0}
\rightarrow\vec{0}
\end{equation}
\end{subequations}
%====================================================
