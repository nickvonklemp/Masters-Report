...
\begin{equation}
\dot{z}_1=\dot{\mathcal{E}}_d-\dot{\mathcal{E}}_b=Q_b^*\otimes \vec{0} \otimes Q_b - Q_b^*\otimes\vec{v}_b\otimes Q_b= - Q_b^*\otimes \vec{v}_b\otimes Q_b
\end{equation}
...
\begin{subequations}
\begin{equation}
z_1\text{}'=X_E=Q_b^*\otimes z_1 \otimes Q_b 
\end{equation}
\vspace{-10pt}
\begin{equation}
\therefore \dot{z}_1\text{}'=Q_b^*\otimes\dot{z}_1\otimes Q_b = -\vec{v}_b
\end{equation}
\vspace{-10pt}
\end{subequations}
Then proposing the following LCF:
\begin{subequations}
\begin{equation}
V_1(z_1\text{}')=\frac{1}{2}z_1\text{}'\text{}^{T}z_1\text{}'
\end{equation}
\vspace{-10pt}
\begin{equation}
\Rightarrow\dot{V}_1=z_1\text{}'\text{}^T\dot{z}_1\text{}'=-z_1\text{}'^T\vec{v}_b
\end{equation}
\end{subequations}
Then, a stabilizing velocity function is\footnote{Using $\Omega_d$ to differentiate from $\vec{v}_d$ which would otherwise be the translational velocity produced from the desired trajectory\ldots}:
\begin{subequations}
\begin{equation}
\Omega_d = \Gamma_1 z_1\text{}'
\end{equation}
\vspace{-15pt}
\begin{equation}
z_2\text{}' = \Omega_d - \vec{v}_b = \Gamma_1z_1\text{}'-\vec{v}_b
\end{equation}
\vspace{-15pt}
\begin{equation}
\therefore \vec{v}_b=\Gamma_1z_1\text{}'-z_2\text{}'
\end{equation}
Which has a derivative:
\begin{equation}
\dot{z}_2\text{}'=\dot{\Omega}_d-\dot{\vec{b}}_b=\Gamma_1\dot{z}_1\text{}'-m^{-1}\big(-\vec{\omega}_b\times m\vec{v}_b+\vec{G}_b+\vec{D}_b+\mu\vec{F}\big)
\end{equation}
So introducing that second error to a new LCF:
\begin{equation}
V_2=\frac{1}{2}z_1\text{}'\text{}^Tz_1\text{}'+\frac{1}{2}z_2\text{}'\text{}^Tz_2\text{}'
\end{equation}
Which has a gradient of:
\begin{equation}
\dot{V}_2=z_1\text{}'\text{}^T\dot{z}_1\text{}'+z_2\text{}'\text{}^T\dot{z}_2\text{}'
\end{equation}
\vspace{-10pt}
\begin{equation}
=-z_1\text{}'\text{}^T\vec{v}_b+z_2\text{}'\text{}^T\bigg(\Gamma_1\dot{z}_1\text{}'-m^{-1}\big(-\vec{\omega}_b\times m\vec{v}_b+\vec{G}_b+\vec{D}_b+\mu\vec{F}\big)\bigg)
\end{equation}
\vspace{-5pt}
\begin{equation}
=-z_1\text{}'\text{}^T\big(\Gamma_1z_1\text{}'-z_2\text{}'\big)+z_2\text{}'\text{}^T\bigg(\Gamma_1\dot{z}_1\text{}'-m^{-1}\big(-\vec{\omega}_b\times m\vec{v}_b+\vec{G}_b+\vec{D}_b+\mu\vec{F}\big)\bigg)
\end{equation}
\vspace{-5pt}
\begin{equation}
=-z_1\text{}'\text{}^T\Gamma_1z_1\text{}'+z_2\text{}'\text{}^T\bigg(-z_1\text{}'-\Gamma_1\vec{v}_b-m^{-1}\big(-\vec{\omega}_b\times m\vec{v}_b+\vec{G}_b+\vec{D}_b+\mu\vec{F}\big)\bigg)
\end{equation}
\end{subequations}
An ideal backstepping control law, with the assumption that $\vec{D}_b$ is known, is then:
\begin{subequations}
\begin{equation}
\mu\vec{F}_{_{IBC}}=\vec{\omega}_b\times m\vec{v}_b-\vec{G}_b-\vec{D}_b-mz_1\text{}'-m\Gamma_1\vec{v}_b-\Gamma_2z_2\text{}'
\end{equation}
\vspace{-10pt}
\begin{equation}
\Rightarrow \dot{V}_{_{IBC}}=\dot{V}_2=-z_1\text{}'\text{}^T\Gamma_1z_1\text{}'-z_2\text{}'\text{}^T\Gamma_2z_2\text{}'\leq\vec{0}~~\forall(z_1,~z_2)
\end{equation}
\end{subequations}
Which obviously leads to asymptotic stability under the assumption that the disturbance is known and can be compensated for well. In the controller both $\Gamma_1$ \& $\Gamma_2$ are P.D symmetric control coefficients to be determined later. Adjusting the backstepping rule and LCF to include a distrubance approximation, similar to the ABC attitude controller in Section:\ref{subsubsec:control.attitude.nonlinear.adaptivebackstep}:
\begin{subequations}
\begin{equation}
1
\end{equation}
