...
\begin{subfigure}{\textwidth}
\centering
\includegraphics[width=0.7\textwidth]{graphs/error_LR}
\vspace{-8pt}
...
\begin{subequations}
\begin{equation}
V_{_{ABC}}= V_{_{IBC}}+\frac{1}{2}\vec{D}_\Delta^{~T}\Gamma_D^{-1}\vec{D}_\Delta=
\frac{1}{2}\hat{z}_1\text{}^T\hat{z}_1+\frac{1}{2}\hat{z}_2\text{}^T\hat{z}_2+\frac{1}{2}\vec{D}_\Delta^{~T}\Gamma_D^{-1}\vec{D}_\Delta
\end{equation}
\vspace{-10pt}
\begin{equation}
...
\vspace{-12pt}
\begin{equation}
\therefore\dot{V}_{_{ABC}}=-\hat{z}_1\text{}^T\Gamma_1\hat{z}-\hat{z}_2\text{}^T\Gamma_2\hat{z}_2-\vec{D}_\Delta^{~T}\Gamma_D^{-1}\big(\hat{D}_{n-1}-\hat{D}_n\big)
\end{equation}
\vspace{-10pt}
\begin{equation}
\equiv-\hat{z}_1\text{}^T\Gamma_1\hat{z}_1-\hat{z}_2\text{}^T\Gamma_2\hat{z}_2-\vec{D}_\Delta^{~T}\Gamma_D^{-1}\vec{D}_\Delta<0~~~~\forall(\hat{z}_1,\hat{z}_2,\vec{D}_\Delta)
\end{equation}
\end{subequations}
The force disturbance observer tracks a generalized single axis disturbance as illustrated in Fig:\ref{fig:disturbance_D}. Note that Fig:\ref{fig:example_D} is in \emph{open loop}. An error deviation for the estimation and the true disturbance is plotted in Fig:\ref{fig:error_DR}. A vector wind field and stochastic force fluctuations are included in the disturbance model, later discussed in Sec:\ref{sec:simulation.disturbance.force}.
\begin{figure}[hbtp]
\vspace{-6pt}
\centering
\begin{subfigure}{\textwidth}
...
\includegraphics[width=0.7\textwidth]{graphs/error_LR}
\vspace{-8pt}
\caption{Disturbance error deviation $\vec{D}_\Delta$}
\label{fig:error_DR}
\end{subfigure}
\vspace{-6pt}
\caption{Adaptive disturbance observer example}
...
\end{figure}
\par
Similarly for the suggested exponentially stable adaptive backstepping controller for the attitude plant, using $\hat{D}_{n-1}$ for an existing disturbance estimate will need to be simulated in order to ascertain if it's a suitable conjecture (Sec:\ref{sec:simulation.disturbance.force})\ldots
