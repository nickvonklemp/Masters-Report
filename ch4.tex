...
%====================================================
Following the higher level control laws, a distribution algorithm is needed to \emph{allocate} out the desired virtual control inputs, $\vec{\nu}_d=[\mu\vec{F}~\mu\vec{\tau}]^T$, to commanded actuator positions, $u\in\mathbb{U}$. The allocator block, $B^\dagger(\mathbf{x},~\vec{\nu}_d,~t)$, from the control loop, Fig:\ref{fig:control-loop}, constructs physical actuator positions from the virtual control input. For a regular unconstrained control allocation problem the cost function to be minimized\footnote{\emph{Control Allocation by Johansen, et al. [2012]\cite{allocation}} and \emph{Control allocation for over actuated systems by Oppenheimer, et al. [2006]\cite{controlallocation}} both detail the nature of generalized nonlinear allocation loops.} is typically some function $J(\mathbf{x},~u,~t)$ representing the magnitude of actuator deflection. The allocation problem is then posed as:
\begin{equation}\label{eq:allocation}
\underset{u\in\mathbb{U}}{min}~J(\mathbf{x},u,t)~~\text{such that}~~\vec{\nu}_c=B(\mathbf{x},u,t)
\end{equation}
In the over actuated case, there exists an entire set of suitable solutions of $u$ to Eq:\ref{eq:allocation}. The allocation rule is then structured as an optimization problem which aims to minimize some slack variable $s$.
\begin{equation}
\underset{u\in\mathbb{U}}{min}\big(||Q_s||+J(\mathbf{x},u,t)\big)~~\text{such that}~~\vec{\nu}_d-\vec{\nu}_c=h(\mathbf{x}_e,t)-B(\mathbf{x},u,t)=s
\end{equation}
Where $||Q_s||$ is a secondary control objective introduced for the slack variable. Not much work has been done on over-allocation for aerospace vehicles outside the field of satellite attitude control. Often satellites are over actuated for the sake of fault tolerance and redundancy.
\par
