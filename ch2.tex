...
\par
A \emph{transformation} changes the vectors reference frame. The transformation is a rotation by an angle which is the difference between the resulting and principle reference frames. A transformation from frame $\mathcal{F}^i$ to $\mathcal{F}^b$ by an angle of $\psi$ about the $\vec{X}$ axis is then:
...
\vspace{-20pt}
\begin{equation}
\mathbb{R}_{i}^{b}\triangleq\mathbb{R}_z(-\phi)\mathbb{R}_y(-\theta)\mathbb{R}_x(-\psi)
\end{equation}
\end{subequations}
\par
