...
\end{equation}
\end{subequations}
$B(x,t,\mathbb{U})$ is the allocation rule and can, if the plant permits it, be abstracted to a multiplicative relationship $B(x,t)\mathbb{U}$ such that; $B(x,t)\in\mathbb{R}^{n\times m}$. Over-actuation is not something often applied to aerospace rotor-craft bodies, the additional actuation introduces a significant weight gain which could jeopardize its flight stability. The only overactuated quadrotor (birotor dual-axis tilting makes the system critically actuated) literature which covers allocation of the given actuators is \cite{tiltgasco,tiltrihani}, where the authors apply a weighted pseudo inverse (sic Moore Penrose Inverse \cite{}, control allocation techniques are covered in Section: \ref{sec:control.allocation}). Normally more applicable to orbital satellite attitude control, there is a variety of literature on actuator fusion to balance control effort between effectors.
\par
Vectored thrust fundamentals can be applied to marine propulsion systems \cite{} but don't often pertain to allocation theory.
%====================================================
