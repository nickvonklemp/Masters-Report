...
\end{table}
\par
In a collection of papers, written by Bouabdallah et al \ldots (2003,2004,2007) arguably the most prolific early quadrotor authors, a range of different control implementations is derived and reviewed. Their last paper (2007)\cite{fullquadcoptercontrol} derived and pratically tested an Integral Backstepping attitude controller on an OS4 quadrotor. This builds on their research from an earlier paper (2003)\cite{pidlqr} wherein an analysis of PID vs LQR attitude controllers in the context of quadrotors is posed. LQR controllers aim to optimize the controller effort ($||\mathbb{U}||$ or the $L_2$ norm of the plant input) and although, in theory, solving the Ricatti equation may produce an optimially stable and efficient control law but it requires exact plant matching. In practice, direct plant identification is difficult to achieve for an aerospace frame.  The resultant controller in \cite{pidlqr} achieved asymptotic stability but performed poorly due to low fidelity of actuator dynamics and poor confidence inertial measurements.
\par
Adaptive Backstepping Control\cite{backstepping} expands on IBC fundamentals by introducing an added estimated disturbance state term in the LCF used for the backstepping iteration. The caveat of using such an approach is that, from the Lyupanov control theorem, a time derivative for the estimated disturbance (or \emph{update law}) is needed. The complexity involved in disturbance approximation for a signal without any apriori approximations is that some heuristic needs to be adopted with the approximation. In \cite{nonlinearadaptive} a stastical $proj(.)$ operator is applied to changes in estimate update laws but still r
\par
A PID strucuture for attitude controllers are the norm, with \cite{optimizedPID},\cite{quaddynamics},\cite{tiltpropellerflight} all implementing standard PID controllers and even \cite{singleaxistilting} using only a PD controller despite having an over-actuated platform. 
%====================================================
