...
\\
%===================NEEDS=WORK======================
One approach to improving quadrotor flight operation is to alter the manner in which thrust is altered. Drawing from helicopter technology, some papers \cite{} are now using swashplates to vary the propeller pitch for a faster bandwidth'ed actuator. To otherwise 

An interesting project is that of Svein Rivli Napsholm, \cite{}, whom designed and prototyped a wholly unique quadrotor helicopter. The outcome of the design was a cyclic-pitch actuated propeller mechanism which then has its own axis of actuation. The mechanically complex rotor bracket (Fig:\ref{fig:tiltrotor-naspholm}) applied an actuated swash plate to control propeller pitch, and hence thrust. The author deliberated a lot about the aerodynamic effects, more commonly associated with helicopter dynamics, which would affect his proposed prototype. The final implementation was an elegant design which used off-the-shelf components and some custom fabricated parts to achieve the desired actuation. Finally, the control design for the project was that of the model predictive control variety. Despite it being what would appear to be the first MPC based quadrotor project, the control was bereft of detail and clearly cut the research short due to time constraints.
%=================================================
\par
...
\par
Quadrotor plant dynamics, as has been mentioned before, are often linearised, especially when represented with a 3-variable Euler angle set, $E = [\phi ~\theta ~\psi]^T$. The coupled gyroscopic and Coriolis responses are both neglected when the angular velocity rate is small, $\vec{\Omega} \approx 0$, and the inertial matrix is diagonal, $rk(\mathbb{I}_b)= x$ for $\in\mathbb{R}^x$. The consequence of which is the ineffectual deterioration of both the gyroscopic term, $\tau_{gyro}=\vec{\Omega} \times \mathbb{I}_b\vec{\Omega} \approx 0$ and the  Coriolis force term, $F_{cor}=-\vec{\Omega} \times \vec{a_b} \approx 0$ in the bodies dynamics (Chapter:\ref{ch:dynamics} for context). Once the cross-product coupled terms are no longer of consequence, each of the 6 degrees of freedom, $[X ~Y ~Z]^T, [\phi ~\theta ~\psi]^T$, can be treated as an individual SISO plant with appropriate techniques used.
\par
%====================================================
