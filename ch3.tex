...
\end{subequations}
\newpage
Noting that $\mathbb{I}$ is the inertial tensor aligned w.r.t to whichever generalized coordinates are being used. The Euler-Lagrange formulation equates partial derivatives of the Lagrangian to any generalized forces, $\mathbf{V}$, acting on the system.
...
\end{equation}
\end{subequations}
In any generalized coordinate system a rotating vector's time derivative \cite{reynolds,conservationequations} is given by:
\begin{equation}\label{eq:reynolds}
\frac{d\vec{f}}{dt_a}=\frac{d\vec{f}}{dt_b}+\vec{\omega}_{a/b}\times\vec{f}
\end{equation}
So applying the Reynolds Transportation Theorem (Eq:\ref{eq:reynolds}) to the partial derivatives in Eq:\ref{eq:partial.b} and further defining the generalized co-ordinates as cartesian body coordinates with respect to inertial origin. Noting that in Eq:\ref{eq:partial.b} the place holders used for linear and angular positions are in a specific frame, $[ \dot{\mathcal{E}}~~\dot{\eta} ]^T \in \mathcal{F}^b$, and hence $\equiv [ \nu ~~ \omega]^T$. It then follows that Lagrangian will change:
\begin{subequations}
\begin{equation}
...
\end{equation}
\end{subequations}
Which, when reintroduced to the Euler-Langrage formulation Eq:\ref{eq:euler-lagrange}, results in the familiar Newton-Euler equations for linear and angular movements, in the body frame;
...
\begin{equation}
\dot{\mathcal{E}}=\mathbb{R}_b^I\nu~~~~\in\mathcal{F}^I
\end{equation}
\vspace{-15pt}
\begin{equation}
F=m\dot{\nu}+\omega_b\times m\nu -m \mathbb{R}_I^b g ~~~~\in\mathcal{F}^b
\end{equation}
\vspace{-15pt}
\begin{equation}
...
\end{equation}
\end{subequations}
The set of state equations Eq:\ref{eq:states} could be reduced to a set of two equations, only in their respective reference frames of the state variables which they describe. The non-linear form of those equations substitutes $\dot{\eta}=\Phi(\eta)\omega_b$ in the Lagrangian derivative, Eq:\ref{eq:partial.b}.
\begin{equation}
...
\end{equation}
This only changes the angular component, and so applying the chain rule:
\begin{equation}
\mathbb{I}\frac{d}{dt}\Phi(\eta)\dot{\omega}_b=\mathbb{I}\big(\Phi\dot{(\eta)}\omega_b+\Phi(\eta)\dot{\omega}_b \big)
\end{equation}
Drawing from \cite{autonomousrobotseuler} and recognizing that $\mathbb{I}$ must be transformed to common axes, $J=\Psi(\eta)^T\mathbb{I}\Psi(\eta)$. The controllable differential equations in Eq:\ref{eq:newton}, then in the inertial frame for force and intermediate Euler frames for each angle becomes:
...
\end{equation}
\end{subequations}
Equation \ref{eq:nonlinear} fully describes the state derivative $\ddot{\eta}$ in its own frame(s). The two differential equations which dictate the entire bodies motion are:
\begin{subequations}
\begin{equation}
F=m\dot{\mathcal{E}}+\mathbb{R}_b^I\omega_b \times m \dot{\mathcal{E}}-mg~~~~\in\mathcal{F}^I
\end{equation}
\vspace{-10pt}
\begin{equation}
...
The generalized forces effecting the system, $F$ and $\tau$, are the system's controllable inputs and are going to be directly related to the actuator effectiveness terms. In the general case, which is expanded upon in Section:\ref{sec:dynamics.aero}, the control inputs are effected as follows:
\begin{subequations}
\begin{equation}
\mu F = \sum \vec{T}_i
\end{equation}
