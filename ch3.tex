...
\label{ch:dynamics}
%====================================================
Generally applicable rigid body dynamics are first derived. Thereafter, those dynamics are refined to the non-linear multi-body case where restricted relative rotational responses are accounted for. Following that non-linear aerodynamic effects are investigated and incorporated into the plant's model. Finally a consolidated, quaternion based plant model is presented which is used for the later control plant development next in Chapter:\ref{ch:control}.
%====================================================
...
%====================================================
The inertial equations presented first are all consequences of design decisions taken. As a result they're an extension of that design in Section:\ref{sec:proto.design} but are pertinent to the following nonlinear multibody dynamics derived next. Although inertias are presented here rounded to either 2 or 0 decimal places, full floating point numbers are used in simulation and prototype software. In some cases when transforming inertias it's more appropriate to use rotation matrices to enact the transformation and not quaternions.
%====================================================
...
\end{equation}
\par
Variable inertias dependent on state input variables are the first of many non-trivial aspects unique to the aircraft's multi-body interconnected design. The resultant control solutions are thus decidedly plant dependent in their formulation. Secondly, the center of mass for each motor module's compound assembly isn't coincidental with either rotational axes intersection. As a result the effective center of mass for each module, and the entire structure as well, is going to be a function of the angular rotational position of each motor module and time varying.
\par
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figs/inertia-center}
...
\end{equation}
Making the resultant gravitational torque\footnote{With $\vec{G}_b=Q_b^*\otimes\vec{F_g}\otimes Q_b~~[N]$} about the origin $\vec{\mathbf{O}}_b$ at any given moment:
\begin{equation}
...
\par
Each of the four motor modules are symmetrical and as such the induced torque response characteristics for one module can be extrapolated through a reference frame rotation. Seeing that each relative rotation from the actuator set $u\in\mathbb{U}$ is induced independently and upon a different body, their responses are calculated separately too. Drawing again from Lagrangian theory\footnote{Generalized linear kinetic energy is considered on the whole body in Eq:\ref{eq:rigid-frame.a} and remains unchanged} and considering only the rotational kinetic energy for the inner ring assembly $\mathcal{F}^{M_i}$, there is no permissible translational motions between each body and as such there is no linear kinetic energy contribution.
\par
The Lagrangian for the inner ring is formed:
\begin{equation}
...
\end{equation}
Where $\mathbb{I}_p$ is the propellers rotational inertia about it's $\hat{Z}$ axis and $\mathbb{I}_\lambda$ being defined in Eq:\ref{eq:inertia.inner.a}. Noting that $\vec{\Omega}_i=[0~~0~~\Omega_i]^T\in\mathcal{F}^{M_i}$ and $\dot{\vec{\lambda}}_i=[\dot{\lambda}_i~~0~~0]^T\in\mathcal{F}^{M_i'}$, the two contributors are not in a common frame.
\begin{subequations}
\begin{equation}
\mathcal{L}_{M_i}=\frac{1}{2}\vec{\Omega}_i^{~T}\big(\mathbb{I}_{p}\big)\vec{\Omega}_i+\frac{1}{2}\dot{\vec{\lambda}}_i'^{~T}\big(\mathbb{I}_{\lambda}\big)\dot{\vec{\lambda}}_i'
\end{equation}
With a transformed angular rate\footnote{$\dot{\vec{\lambda}}_i$ is superfluous but included for completeness}:
\begin{equation}
...
\end{equation}
\end{subequations}
And both $\mathbb{I}_{p}$ and $\mathbb{I}_{\lambda}$ being taken W.R.T to their centre(s) of rotation. Recalling the Euler-Lagrange formulation from Eq:\ref{eq:euler-lagrange} with the same generalized co-ordinates $\mathbf{r}(t)$ of $\mathcal{F}^b$ relative to $\mathcal{F}^I$
\begin{equation}
\frac{d}{dt}\bigg(\frac{\delta L}{\delta \dot{\mathbf{r}}}\bigg)-\frac{\delta L}{\delta \mathbf{r}} = \mathbf{V} = \vec{\tau}_{net}
\end{equation}
Then:
\begin{subequations}
\begin{equation}
\frac{d}{dt_b}\bigg(\frac{\delta\mathcal{L}}{\delta\dot{\mathbf{r}}}\bigg)=\frac{d}{dt_{M_i}}\mathbb{I}_p\vec{\Omega}_i+\vec{\omega}_{M_i/I}\times\mathbb{I}_p\vec{\Omega}_i+\frac{d}{dt_{M_i}}\mathbb{I}_{\lambda}\dot{\vec{\lambda}}_i+\vec{\omega}_{M_i/I}\times\mathbb{I}_{\lambda}\dot{\vec{\lambda}}_i
\end{equation}
\begin{equation}
\vec{\tau}_\lambda=\underbrace{\mathbb{I}_p\dot{\vec{\Omega}}_i+\mathbb{I}_{\lambda}\ddot{\vec{\lambda}}_i}_{Inertial}+\underbrace{\vec{\omega}_{M_i/I}\times\mathbb{I}_p\vec{\Omega}_i+\vec{\omega}_{M_i/I}\times\mathbb{I}_{\lambda}\dot{\vec{\lambda}}_i}_{Gyroscopic}~~~~\in\mathcal{F}^{M_i}
\end{equation}
Where $\vec{\omega}_{M_i/I}$ is the angular velocity of the inner ring frame with respect to the inertial frame:
\begin{equation}
\vec{\omega}_{M_i/I}=\mathbb{R}_z\mathbb{R}_y(\alpha_i)\mathbb{R}_x(\lambda_i)\vec{\omega}_b+\mathbb{R}_y(\alpha_i)\mathbb{R}_x(\lambda_i)\dot{\vec{\alpha}}_i+\mathbb{R}_x(\lambda)\dot{\vec{\lambda}}_i
\end{equation}
\end{subequations}
Similarly for the middle ring response:
\begin{subequations}
\begin{equation}
\mathcal{L}_{M_i'}=\frac{1}{2}\dot{\vec{\alpha}}_i^{~T}\big(\mathbb{I}_{\alpha}(\lambda_i)\big)\dot{\vec{\alpha}}_i
\end{equation}
\begin{equation}
\vec{\tau}_\alpha=\underbrace{\mathbb{I}_{\alpha}(\lambda_i)\ddot{\vec{\alpha}}_i}_{Inertial}+\underbrace{\vec{\omega}_{M_i'/I}\times\mathbb{I}_{\alpha}(\lambda_i)\dot{\vec{\alpha}}_i}_{Gyroscopic}~~~~\in\mathcal{F}^{M_i'}
\end{equation}
\begin{equation}
\vec{\omega}_{M_i'/I}=\mathbb{R}_z\mathbb{R}_y(\alpha_i)\vec{\omega}_b+\mathbb{R}_y(\alpha_i)\dot{\vec{\alpha}}_i
\end{equation}
\end{subequations}
