...
\end{figure}
Following the same process to evaluate the middle ring's response, but with respect to the intermediary frame $\mathcal{F}^{M_i''}$. The entire motor module assembly (Fig:\ref{fig:response-middle}) consists of three independent bodies; the rotor assembly, inner ring (again without rotor contributions) and finally the middle ring assembly. The rotor assembly has a transformed inertia:
\begin{subequations}
\begin{equation}
