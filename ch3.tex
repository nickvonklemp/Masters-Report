...
\end{equation} 
Which similarly has a derivative $\dot{J}_{ir}''$:
...
\end{equation}
\end{subequations}
Where $J_{p}$ is the collective motor module inertia from Eq:\ref{eq:inertia.middle.b}. Each body then has its own relative angular velocity with respect to the frame $\mathcal{F}^{M_i''}$. For the rotor; $\vec{\omega}_{r/M_i''}$ is the relative angular velocity of that assembly:
...
\begin{equation}
\vec{\omega}_{M_i/M_i''}=\dot{\vec{\alpha}}_i+R_y(\alpha)\dot{\vec{\lambda}}_i=\dot{\vec{\alpha}}_i+\dot{\vec{\lambda}}_i\hspace{-2pt}'
\end{equation}
\vspace{-16pt}
\begin{equation}
...
\begin{multline}
\vec{\mathbf{V}}=\Big(\dot{J}_r''-R_y(\alpha)\big(\dot{J}_r'\big)R_y^{-1}(\alpha)\Big)\dot{\vec{\alpha}}_i+\big(J_r''\big)\ddot{\vec{\alpha}}_i+\dot{\vec{\alpha}}_i\times\big(J_r''\big)\Big(\vec{\Omega}_i\hspace{-2pt}''+\dot{\vec{\lambda}}_i\hspace{-2pt}'+\dot{\vec{\alpha}}\Big)
\\
...
\end{multline}
Which can be further simplified by introducing combined inertial bodies $J_n=J_r+J_{ir}$ for the \emph{entire} inner ring, Eq:\ref{eq:inertia.inner}, and $J_p=J_m+R_x(\lambda)\big(J_n\big)R_x^{-1}(\lambda)$ for the entire motor module's inertia, Eq:\ref{eq:inertia.middle.b}.
\begin{equation}\
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{figs/response-body}
