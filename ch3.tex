...
Each unit quaternion, stemming from Euler-Rodriguez theorem, is parameterized such that the quaternion operation represents a single rotation of $\theta$ about an axis $\hat{u}$ such that:
\begin{equation}
Q=\begin{bmatrix}
q_0\\
\vec{q}
...
\end{equation}
\end{subequations}
Every physical attitude in $\mathbb{R}^3$ has two corresponding quaternions in $\mathbb{R}^4$; $[q_0~\pm\vec{q}]^T$. Unless this dual coverage is incorporated into the control plant, the error state 

could  two possible attitude differences. One through a clock-wise $\theta$ rotation and another through an anti-clockwise $2\pi-\theta$ rotation. The control plant may erroneously command an anti-clockwise rotation through $2\pi-\theta$ even if $\theta$ may be a better trajectory.
%****************************************************
