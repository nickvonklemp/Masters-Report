...
%====================================================
\subsection{Induced model simulation and verification}
\label{subsec:dynamics.nonlinearities.torque-tests}
%====================================================
\subsubsection{Simulation of model}
...
\frac{d}{dt}\bigg(\frac{\partial\mathcal{L}_{M_i'}}{\partial\dot{\vec{\mathbf{u}}}}\bigg)-\frac{\partial\mathcal{L}_{M_i'}}{\partial\vec{\mathbf{u}}}=\vec{\mathbf{U}}~~\text{and}~~\frac{d}{dt}\bigg(\frac{\partial\mathcal{L}_{M_i''}}{\partial\dot{\vec{\mathbf{v}}}}\bigg)-\frac{\partial\mathcal{L}_{M_i''}}{\partial\vec{\mathbf{v}}}=\vec{\mathbf{V}}
\end{equation}
The assumption in Eq:\ref{eq:rotation-linear}, presented in \cite{rotationlinearize}, is applied to linearize and simplify partial derivatives of the respective inner and middle ring Lagrangians in Eq:\ref{eq:sim-lagranges}. Those simplifications are such that Eq:\ref{eq:sim-lagranges} becomes:
\begin{equation}
...
\end{equation}
Testing that assumption, presented in Eq:\ref{eq:rotation-linear}; the partial derivatives of the rotor's transformed inertia in Eq:\ref{eq:sim-lagrange-inner-expand}, with respect to $\vec{\lambda}_i$, are as per the product rule:
\begin{subequations}
\begin{equation}
...
\end{equation}
Similarly for the inner ring's contribution in Eq:\ref{eq:sim-lagrange-inner-expand} without the rotor body's inertia:
...
\frac{\partial}{\partial\vec{\lambda}_i}\Big(R_x(\lambda)\Big)\vec{\Omega}_i
\end{equation}
\end{subequations}
...
\end{equation}
\end{subequations}
It can therefore be said that the assumption in Eq:\ref{eq:sim-lagrange-inner-simple} is fair, or that:
\begin{equation}
...
\end{equation}
For a final test of those simplifications, a \textbf{Simulink} loop is made which applies a direct comparison of $\vec{\tau}_\lambda$, from Eq:\ref{eq:torque-induced-inner}, both with and without the rotation matrix linearization. Convention has it that an estimated or simulated values are denoted with a hat accent. Therefore a calculated inner ring torque response \emph{with linearized} partial derivatives as per Eq:\ref{eq:inner-ring-linearizations} is termed as $\hat{\tau}_\lambda$. The true representation of the induced torque response, $\vec{\tau}_\lambda$ is calculated from the following block model illustrated in Fig:\ref{fig:linear-block}.
\par
\begin{figure}[hbtp]
\centering
...
\end{figure}
Where the block $u(t)$ generates a commanded change in actuator position, $\Delta u$, which affects the actuator dynamics $P(s)$ detailed in Sec:\ref{subsec:proto.design.transfer}. That argument $u$ is then used to calculate the full Lagrangian:
\begin{equation}
...
\end{equation}
Fig:\ref{fig:tau-lamba-hat} shows plots for both estimated $\hat{\tau}_\lambda$ and simulated $\vec{\tau}_\lambda$. Various steps up from a starting constant $\lambda(0)$ to set points are shown in Fig:\ref{fig:tau-lamba-hat-up} whilst Fig:\ref{fig:tau-lmabda-hat-down} shows steps down from starting points to $\lambda(0)$. The variation from $\hat{\tau}_\lambda$ to $\vec{\tau}_\lambda$ is an order of magnitude smaller than the peak of either signals. 

an down from a starting $\lambda_i(0)$.
\subsubsection{Dynamic model verification}
extended to partial derivatives 


 generalize path co-ordinates $\vec{\mathbf{v}}$ solves for generalized forces $\vec{\mathbf{V}}$ acting on the system as follows:
