...
\end{figure}
\par
The differential torque about the $\hat{Z}_b$ axis only affects the attitude's yaw angle $\psi$. Fig:\ref{fig:spin} shows how that yaw torque, created by redirecting motor module's thrust vectors, produces an oscillatory motion in the attitude plant. The reduced lift force from redirection of the two motor module thrust vectors causes the vehicle to slowly descend from its hovering height of $5~[\text{m}]$, shown in Fig:\ref{fig:spin_position}. This particular test shows that minor perturbations away from a stable hover point cause large deviations in state variables, especially in the attitude plant. This indicates that open loop hovering stability is extremely fragile and necessitates the need for closed loop position control to achieve the desired goal of attitude and position setpoint tracking.
\begin{figure}[htbp]
\centering
\vspace{-8pt}
...
\end{figure}
\par
Whilst none of the simulated results are wholly unexpected, especially considering all the dynamics are derived from established fundamental theorems, the above simulations and physical tests performed do provide a degree of certainty to proposed model for the multibody system. The only way to ascertain the absolute accuracy of the equations of motion is to compare them to physical flight test data which is, however, beyond the scope of this investigation.
%====================================================
