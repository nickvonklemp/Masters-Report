...
\end{equation}
\end{subequations}
Unfortunately that inertial rate, Eq:\ref{eq:module-point-mass-rate}, is not able to be simplified to a more concise form. The Lagrangian for the energy of a single motor module, about the origin $\vec{\mathbf{O}}_b$, $\mathcal{L}_{p_i}$ can then be constructed. This time, including the gravitational potential energy component:
\begin{multline}
\mathcal{L}_{p_i}=\frac{1}{2}\vec{\omega}_{r/I}^T\big(J_r'''\big)\vec{\omega}_{r/I}+\frac{1}{2}\vec{\omega}_{M_i/I}^T\big(J_{ir}'''\big)\vec{\omega}_{M_i/I}+\frac{1}{2}\vec{\omega}_{M_i'/I}^T\big(J_m''\big)\vec{\omega}_{M_i'/I}+\vec{\omega}_{b/I}^T\big(J_H)\vec{\omega}_{b/I}\\-m_b\big(R_I^b(\eta)\vec{\mathcal{E}}+\Delta L_i\big)\cdot\vec{G}_b
\end{multline}
...
\\
-m_b\big(R_I^b(\eta)\vec{\mathcal{E}}+\Delta L_i\big)\cdot\vec{G}_b
\end{multline}
