...
\end{equation}
\newpage
That in-plane fluid flow $\vec{U}(r,\phi)$, for an element at radius $r$ with a local angle of attack $\phi$, then contributes towards elemental lift and drag forces as a function of dimensionless lift, $C_L$, and drag, $C_D$, coefficients.
\begin{subequations}
\begin{equation}
...
\end{equation}
\end{subequations}
Then calculating forces and torques as per momentum theory for each element, in terms of tangential and axial induction factors:
\begin{subequations}
\begin{equation}
...
\vspace{-10pt}
\begin{equation}
dQ=\rho 4 \pi r^3 v_\infty(1+a)a'.dr
\end{equation}
\end{subequations}
Then equating momentum and element terms together produces the blade-element momentum equation(s) for thrust and torque produced by a propeller. There are a few assumptions made:
\begin{itemize}
\item The tangential induction factor, $a'$, is small compared to the propellers rotational velocity $\Omega r(1+a')\approx \Omega r$.
\item $\phi <<1$, or that the net fluid inflow $v_b$ is small when compared to the rotational velocity $\Omega r$. As such the following small angle approximations are made with regards to the local angle of attack:
\begin{itemize}
\item 
\end{itemize}
\end{itemize}
