...
\end{equation} 
Where $J_n$ or $J_{M_i}$ is the net inertia for the inner ring assembly, defined numerically in Eq:\ref{eq:inertia.inner}. The rotor assembly has an angular velocity $\vec{\omega}_{r/M_i'}$ relative to the middle ring frame $\mathcal{F}^{M_i'}$ from the BLDC motor's rotation $\Omega_i$ and the inner ring's servo rate $\dot{\lambda}_i$:
...
\vec{\mathbf{U}}=\frac{d}{dt}\bigg(\frac{\partial \mathcal{L}_{M_i'}}{\partial \dot{\vec{\mathbf{u}}}}\bigg)-\frac{\partial \mathcal{L}_{M_i'}}{\partial \vec{\mathbf{u}}}~~~~\in\mathcal{F}^{M_i'}
\end{equation}
...
\therefore\vec{\mathbf{U}}\approx\frac{d}{dt}\bigg(\frac{\partial \mathcal{L}_{M_i'}}{\partial \dot{\vec{\mathbf{u}}}}\bigg)=\frac{d}{dt}\bigg(\big(J_{r}'\big)\Big(R_x(\lambda)\vec{\Omega}_i+\dot{\vec{\lambda}}_i\Big)+\big(J_{ir}'\big)\dot{\vec{\lambda}}_i\bigg)
\end{equation}
...
\end{equation}
\end{subequations}
Where $\dot{\vec{\theta}}=\dot{\theta}\cdot\hat{u}$ is the angular velocity vector between the two frames. In most cases, the inertia won't be changing in its principle frame, or rather that $\dot{J}=0$. Both the rotor assembly and inner ring inertias are constant in their principle frames, the transformed inertias however have the following derivatives:
...
\end{multline}
Recombining inertial bodies with the same angular velocity ($J_{r}'+J_{ir}'=J_{n}'$) and recognizing that, from Eq:\ref{eq:angular-inner}, $\vec{\omega}_{M_i/M_i'}=\dot{\vec{\lambda}}$; the generalized net torque encountered by a rotation of $\lambda$ is:
\begin{subequations}
\begin{equation}
\therefore\vec{\mathbf{U}}=\big(\dot{J}_r'\big)\vec{\Omega}_i'+\big(J_{r}'\big)\dot{\vec{\Omega}}_i\text{}\hspace{-2pt}'+\dot{\vec{\lambda}}_i\times \big(J_{r}'\big)\vec{\Omega}_i\text{}\hspace{-2pt}'+\big(\dot{J}_{n}'\big)\dot{\vec{\lambda}}_i+\big(J_{n}'\big)\ddot{\vec{\lambda}}_i+\dot{\vec{\lambda}}_i\times \big(J_{n}'\big)\dot{\vec{\lambda}}_i~~~~\in\mathcal{F}^{M_i'}
\end{equation}
...
\end{multline}
Again, justifying the rotation matrix linearization using Eq:\ref{eq:rotation-linear}, matrices $J_r''$, $J_{ir}''$ and $J_m'$ are all instantaneous transformed inertias. The Euler-Lagrange formulation then simplifies, with the partial derivative $\partial\mathcal{L}_{M_i''}/\partial\vec{\mathbf{v}}\approx 0$. So the generalized forces $\vec{\mathbf{V}}$ are:
\begin{equation}
\vec{\mathbf{V}}=\frac{d}{dt}\Bigg(\frac{\partial\mathcal{L}_{M_i''}}{\partial\dot{\vec{\mathbf{v}}}}\Bigg)-\frac{\partial\mathcal{L}_{M_i''}}{\partial\vec{\mathbf{v}}}\approx\frac{d}{dt}\Bigg(\frac{\partial\mathcal{L}_{M_i''}}{\partial\dot{\vec{\mathbf{v}}}}\Bigg)~~~~\in\mathcal{F}^{M_i''}
\end{equation}
...
\rightarrow \vec{\mathbf{V}}=\frac{d}{dt}\Bigg(\frac{\partial \mathcal{L}_{M_i''}}{\partial\dot{\vec{\mathbf{v}}}}\Bigg)=\Big[\big(\dot{J}_r''\big)(\vec{\Omega}_i\hspace{-2pt}''+\dot{\vec{\lambda}}_i\hspace{-2pt}'+\dot{\vec{\alpha}}_i)\Big]
\\
...
\rightarrow\vec{\mathbf{V}}=R_y(\alpha)\vec{\mathbf{U}}+\Big(R_y(\alpha)\big(\dot{J}_n'\big)R_y(\alpha)\Big)\dot{\vec{\alpha}}_i+ \Big(\dot{J}_p'-R_y(\alpha)\big(\dot{J}_p\big)R_y^{-1}(\alpha)\Big)\dot{\vec{\alpha}}_i
\\
...
\end{multline}
The servo response $\hat{\tau}_\alpha(\lambda)$ as a function with an argument of the inner ring's servo position is \underline{\emph{not}} the same as the generalized torque $\vec{\mathbf{V}}$ described in Eq:\ref{eq:3.86g}. The latter contains terms for the inner ring's servo response. A hat accent is used again to denote the torque response seeing that is an estimation, not a true measurement. Careful inspection could have yielded the inertial and gyroscopic components of both Eq:\ref{eq:torque-induced-inner} and Eq:\ref{eq:torque-induced-middle}, however the effect of inertial rates on the torque system is a far less obvious result. The assumption in Eq:\ref{eq:rotation-linear} that rotated inertias can be linearized is shown to hold true next in Sec:\ref{subsec:dynamics.nonlinearities.torque-tests} where simulations and physical tests corroborate the above models.
\par
...
\end{equation}
The final non-trivial torque term associated with the multibody motion which needs to be accounted for is as a result of the entire system's motion. Specifically considering the responses which relative rotations $\lambda_i$ and $\alpha_i$ have to the net angular velocity of the entire multibody system $\vec{\omega}_b$. Such responses are an extension of the fundamental rigid 6-DOF differential equation for angular motion, reiterated from Eq:\ref{eq:angular-multibody}:
\begin{equation}
...
\end{figure} 
\par
A massless rod of length $r$ connects some rotational body, with a mass $m_b$, at point $\mathbf{B}$ to a center point $\mathbf{O}$. The principle frame $\mathcal{F}^{1}$ has axes $\hat{X}_1$ and $\hat{Y}_1$ as illustrated. The arm has a rotational velocity $\dot{\phi}$ relative to $\mathcal{F}^{1}$, applied by some "motor". Attached to the end of the rod is a secondary frame $\mathcal{F}^2$ with an $\hat{X}_2$ axis co-linear to the rod and a perpendicular $\hat{Y}_2$. The rotational body, centered at point $\mathbf{B}$, has a rotational inertia $J_\mathbf{B}$ about the point (or axis) $\mathbf{B}$. That rotating body has a rotational velocity $\dot{\theta}$ from another "motor" relative to $\mathcal{F}^2$. The question is then how to find the net torque acting on the system in terms of angular velocities $\dot{\phi}$ and $\dot{\theta}$ and their derivatives (or accelerations)?
\begin{figure}[htbp]
\centering
\begin{subfigure}{0.49\textwidth}
\centering
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=0.65\textwidth]{figs/vector-diff}
...
\end{equation}
A final verification of suggest simplifications is done in a \textbf{Simulink} model. The model applies a direct comparison of $\vec{\tau}_\lambda$, originally from Eq:\ref{eq:torque-induced-inner}, both \emph{with} and \emph{without} the rotation matrix linearization. Convention has it that an estimated or simulated value is denoted with an hat accent. Therefore a calculated inner ring torque response \emph{with linearized} partial derivatives as per Eq:\ref{eq:inner-ring-linearizations} is termed as $\hat{\tau}_\lambda$. The true representation of the induced torque response, $\tilde{\tau}_\lambda$, is calculated from the block model detailed in Fig:\ref{fig:linear-block}. The simulated $\tilde{\tau}_\lambda$ applies no partial derivative simplifications or linearizations, calculating the full Euler-Lagrange formulation from Eq:\ref{eq:sim-lagranges}.
\begin{figure}[htbp]
\centering
...
\end{figure}
\par
The block $u(t)$ in Fig:\ref{fig:linear-block} represents some commanded change in actuator position, with the actuator space $u\in\mathbb{U}$ consisting of propeller speeds $\Omega_i$ and servo positions $\lambda_i$ and $\alpha_i$ for $i\in[1:4]$; defined in Eq:\ref{eq:actuator-space}. Each actuator has its own transfer function, driven by the dynamic block $P(s)$ which is expanded on in Sec:\ref{subsec:proto.design.transfer}. That argument $u(t)$ then leads to some inner ring servo position $\lambda_i(t)$ which can then be used to calculate the complete Euler-Lagrangian equation:
...
\end{equation}
Seeing that Eq:\ref{eq:sim-lagrange-torque-inner} produces a three dimensional vector result and not a scalar; vector magnitudes $\norm{\hat{\tau}_\lambda}$ and $\norm{\tilde{\tau}_\lambda}$ are considered. The plot in Fig:\ref{fig:tau-lambda-hat} shows both estimated $\norm{\hat{\tau}_\lambda}$ and simulated $\norm{\tilde{\tau}_\lambda}$ torques, calculated with a nominal constant angular propeller speed $\vec{\Omega}_i=6000~[\text{RPM}]$. Increasing step sizes for changes in $\Delta\lambda$ are shown, resulting in larger torque responses.  
\begin{figure}[hbtp]
\centering
\begin{subfigure}{0.49\textwidth}
\centering
\includegraphics[width=0.95\textwidth]{graphs/tau-lambda-hat}
\caption{$\norm{\hat{\tau}_\lambda}$ and $\norm{\tilde{\tau}_\lambda}$ steps}
\label{fig:tau-lambda-hat}
\end{subfigure}
\begin{subfigure}{0.49\textwidth}
\includegraphics[width=0.95\textwidth]{graphs/tau-lambda-hat-r}
...
\end{figure}
\par
The regressive deviation $R^{2}_\lambda$ between $\hat{\tau}_\lambda$ and $\tilde{\tau}_\lambda$ is shown in Fig:\ref{fig:tau-lambda-hat-r}. The difference between $\hat{\tau}_\lambda$ and $\vec{\tau}_\lambda$ is exactly the partial derivative contribution $\partial\mathcal{L}_{M_i'}/\partial\vec{\mathbf{u}}$. However the plot for $R^2_\lambda$ suffered from simulation tolerance errors due to the small deviations which were being calculated. However despite that, the difference when using the inertial matrices and dimensions defined for the prototype in Sec:\ref{sec:proto.inertia}, is typically in the order $\times 10^{-5}~~[\text{Nm}]$ for small steps in $\Delta\lambda$. Both $\hat{\tau}_\lambda$ and $\vec{\tau}_\lambda$ were three orders of magnitude greater; torques in the order of $\times 10^{-2}~~[\text{Nm}]$.
\par
\begin{figure}[htbp]
\centering
\begin{subfigure}{0.49\textwidth}
...
\end{figure}
The same process is then applied to the middle ring Lagrangian, $\mathcal{L}_{M_i''}$ from Eq:\ref{eq:sim-lagrange-middle-expand}, to evaluate $\hat{\tau}_\alpha(\lambda)$. Those results are plotted collectively in Fig:\ref{fig:tau-alpha-sim}. Note that both $\norm{\hat{\tau}_\alpha}$ and $\norm{\hat{\tau}_\alpha}$ are plotted, \underline{not the generalized forces} $\vec{\mathbf{V}}$ acting on the system. The generalized force response $\vec{\mathbf{V}}$ from Eq:\ref{eq:sim-lagranges} and expanded in Eq:\ref{eq:3.86g} \underline{include the inner ring energy component} $\vec{\mathbf{U}}$ or $\vec{\tau}_\lambda$ whereas $\hat{\tau}_\alpha$ and $\tilde{\tau}_\alpha$ do not. Moreover only constant values for $\lambda_i$ are used. The same constant propeller speed $\vec{\Omega}_i=6000~~[\text{RPM}]$ was used together with a constant $\lambda_i=0~~[\text{rad}]$ position for the inner ring's servo.
\par
The initial torque spike for $\tau_\alpha$ shown in Fig:\ref{fig:tau-alpha-hat} is as a result of the larger rotational inertia $J_{p}$ encountered by the second order angular acceleration $\ddot{\alpha}$. The response is depreciated in the case of the inner ring for $\tau_\lambda$ because of how much smaller that rotational inertia $J_n$ is. As per power calculations from the servo; the angular acceleration is rate limited to $472~[\text{rad.s}^{\text{-}2}]$, but neither tests come close to reaching saturation.
\par
The second torque peak which begins to manifest in the inner ring response for $\tau_\lambda$ shown in Fig:\ref{fig:tau-lambda-hat} are as a result of the angular velocity rate limit being encountered. The same limit is encountered for the middle ring response $\tau_\alpha$ in Fig:\ref{fig:tau-alpha-hat} but is far less significant in relation to the initial torque peak. The devitation between $\hat{\tau}_\alpha$ and $\tilde{\tau}_\alpha$ is only of the order of $10^{-4}~[\text{N.m}]$ whilst typical induced torque values are again three orders of magnitude greater, or of the order $10^{-1}~[\text{N.m}]$. Only at large angular changes for both $\Delta\lambda_i$ and $\Delta\alpha_i$ does the simplification proposed begin to deteriorate.
\par
