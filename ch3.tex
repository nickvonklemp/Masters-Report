...
%====================================================
The relationship between a propeller's rotational speed, $\Omega_i$, and its produced thrust, $\vec{T}(\Omega_i)$, is slightly more nuanced than the quadratic simplification taken at static conditions which most papers puport. The thrust is mostly dependent on the incident air velocity entering the propellers rotational plane, namely the component of the body velocity normal to the propeller's plane. Parallel fluid velocity to the propeller contributes to aerodynamic drag. The combination of aerodynamic Blade-Element\cite{bem,} and fluid-dynamics Momentum (\emph{disc actuator}) theories produces an integral term taken across the propellers length which accurately models the aerodynamic thrust and torque functions. A verbose presentation of all aerodynamic effects experienced by a quadrotor's propeller(s) is detailed in \cite{.}.
%====================================================
\subsection{Propeller Torque and Thrust}
\label{subsec:dynamics.aero.bem}
%====================================================
\emph{\color{Gray} A feasible situation which the prototype could encounter is where the down-wash of one propeller provides to the incident fluid flow for another propeller. Such a situation presents a complex fluid dynamics problem and remains open to further research. To expedite the design process some simplifications are applied to the aerodynamics to construct an approximate model.}
\par
...
\end{equation}
Where $\Delta v$ is the change in velocity added to the fluid by the propeller's rotation. The propeller induces a velocity directly in front of it's rotational plane, $v_i$, such that the net fluid flow into the plane is $v_b=v_i+v_\infty$. Bernoulli's theorem\cite{.} has it that net fluid flow is
...
\end{equation}
\end{subequations}
Blade-element theory analyses incremental aerofoil sections of a propeller profile at some radius $r$.
%====================================================
\subsection{Drag}
