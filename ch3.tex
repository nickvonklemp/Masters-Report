...
\end{equation}
Where $\Delta \vec{v}$ is the change in velocity added to the fluid by the propeller's rotation. The propeller induces a velocity directly in front of it's rotational plane, $\vec{v}_i$, such that the net fluid flow into the plane is $\vec{v}_b=\vec{v}_i+\vec{v}_\infty$. Bernoulli's theorem\cite{.} has it that net fluid flow is
\begin{equation}\label{eq:bernoulli}
\vec{v}_b=\frac{1}{2}(\vec{v}_s-\vec{v}_\infty)=\frac{1}{2}\Delta v
\end{equation}
And as such, per classical Disc Actuator\cite{.} (\emph{momentum}) theory, the thrust acting on the fluid is a function of mass flow rate and change in velocity.
\begin{equation}\label{eq:prop-mass}
\vec{T}=(A_b \vec{v}_b)\Delta v = \rho \pi R_b^2\vec{v}_b \Delta v = \rho \pi R_b^2(\vec{v}_i+\vec{v}_\infty)\Delta v = \frac{1}{2} \rho \pi R_b^2 \Delta v^2
\end{equation}
Indeed Eq:\ref{eq:prop-mass} can be solved as a function of aerodynamic power, $P=\vec{T}\dot \vec{v}$, however the relationship between rotational kinetic power transferred is tenuous at best. In practice, the fluid velocity change over the propeller has an axial and tangential component. There are then axial and tangential induction factors which are:
\begin{subequations}
\begin{equation}
\vec{v}_b=(1+a)\vec{v}_\infty
\end{equation}
\begin{equation}
\vec{v}_s=(1+2a)\vec{v}_\infty
\end{equation}
And a tangential induction factor dependent on rot
\end{subequations}
\par
.
\par

There is an induced fluid velocity directly in front of and behind the propeller's rotational plane. From conservation of mass principle, that induced velocity, $\vec{v}_i$, contributes to the net fluid inflow, $\vec{v}$, entering and leaving the propellers plane such that:
\begin{subequations}
\begin{equation}
...
That velocity (\emph{vector}) has axial and perpendicular components which are related to incident free stream velocity by some induction factor $a$.
