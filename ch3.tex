...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.6\textwidth]{figs/torque-inner}
\caption{Inner ring torque test rig}
\label{fig:torque-inner}
\end{figure}
\par
It is important to mention that whilst the bearing assembly accommodates for the transfer of the servo's rotational torque, the assembly isolates only the $\hat{X}_{M_i}$ component of the induced torque. If $\vec{\tau}_{M_i}\text{}\hspace{-2pt}'$ is the deflection measured, its relationship with the induced torque vector $\vec{\tau}_{M_i}$ is given by:
\begin{equation}
\vec{\tau}_{M_i}\text{}\hspace{-2pt}'=\vec{\tau}_{M_i}\cdot\hat{X}_{M_i}~~~~\in\mathcal{F}^{M_i}
\end{equation}
...
\end{multline}
Where the term $m_\text{n}C.M'_{\text{n}}\times\vec{G}_{M_i}$ is the gravitational torque arm as a result of a rotated center of mass, $C.M_{\text{n}}'$, defined in Eq:\ref{eq:body-net-inner.d}. Fig:\ref{fig:tau-lambda} shows plots for physical tests of $\hat{\tau}_\lambda$ at varying propeller rotational speeds $\Omega_i$. As per convection, in the plot Fig:\ref{}, $\vec{\tau}_\lambda$ represents the phsyically measured torque applied on the test rig in Fig:\ref{fig:torque-inner}. Similarly $\hat{\tau}_\lambda$ is the simulated or \emph{expected} torque as a result of Eq:\ref{eq:model-inner}. Finally $\hat{\tau}_\lambda'$ is a torque simulated in \textbf{MathWorks} SimScape. App:\ref{} details how the SimScape simulation was constructed from a SolidWorks model to math the phsyical system.
\begin{figure}[hbtp]
\centering
\includegraphics[width=0.6\textwidth]{graphs/tau-lambda}
