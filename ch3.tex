...
\end{figure} 
\par
Isolated free body diagrams for each body under consideration are illustrated in Fig:\ref{}. Starting with the rotational body, Fig:\ref{}, the torque acting at point $\mathbf{B}$ is simply an inertial response to combined angular accelerations $\ddot{\theta}$ and $\ddot{\alpha}$:
\begin{equation}
\tau_\mathbf{B}=-\tau_\theta=-J(\ddot{\theta}+\ddot{\phi})
\end{equation}
...
\end{equation}
That torque and force pair, $F_\mathbf{b}$ and $\tau_\mathbf{b}$, and transferred on the massless rod connecting point $\mathbf{b}$ to $\mathbf{O}$, Fig:\ref{}. The net torque acting around point $\mathbf{O}$ is then comprised of three components; inferred torque from $\tau_\mathbf{b}$, a torque arm from force $F_\mathbf{b}$ and an inertial torque reponse to the effective "point-mass" at point $\mathbf{b}$ relative to $\mathbf{O}$:
\begin{equation}
\tau_\mathbf{O}=-\tau_\phi=-\tau_b-F_br\cos{\phi}+m_br^2\ddot{\phi})
\end{equation}
The reponse force $F_\mathbf{O}$ is of no consequence, however the response torque can now be quantified. The "motor" applies a torque $\tau_\phi$ to the rod to induce some angular acceleration, opposed to that is $\tau_\mathbf{O}$ which acts against that "motor". The torque $\tau_\phi$ acting on the system can then be simplified:
\begin{equation}
\tau_\phi=J(\ddot{\theta}+\ddot{\phi})+m_br^2(\ddot{\phi})-m_b\vec{g}r\cos\phi
\end{equation}
 which leads to the lemma:
\begin{lemma}
Torque acting on a non-inertial system of a doubly rotating body can be found as the contribution of the principle rotational inertial response and the transformed inertial contribution of the secondary 
\[\]
\end{lemma}
\par
