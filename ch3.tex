...
\end{multline}
\end{subequations}
Solving for the generalized energy acting on each system requires application of Euler-Lagrange formulation, using partial derivatives relative to generalized path co-ordinates. Both the inner and middle ring systems were defined with relative co-ordinate paths for the angular servo position $\vec{\mathbf{u}}=[\lambda_i~0~0]^T$ and $\vec{\mathbf{v}}=[0~\alpha_i~0]^T$ respectively. The generalized forces for both systems are then:
...
\end{equation}
The assumption in Eq:\ref{eq:rotation-linear}, presented in \cite{rotationlinearize}, is applied to linearize and simplify partial derivatives of the respective inner and middle ring Lagrangians in Eq:\ref{eq:sim-lagranges}. More specifically, in the case of the inner ring Lagrangian $\mathcal{L}_{M_i'}$, the partial derivative with respect to $\vec{\mathbf{u}}$ simplifies:
\begin{equation}
\frac{\partial\mathcal{L}_{M_i'}}{\partial\vec{\mathbf{u}}}=\frac{\partial\mathcal{L}_{M_i'}}{\partial\vec{\lambda}_i}\approx 0
\end{equation}
Under the assumption that, owing to Eq:\ref{eq:rotation-linear}, the partial derivatives of rotation matrices with respect to $\vec{\lambda}_i$ are:
\begin{subequations}
\begin{equation}
\frac{\partial}{\partial\vec{\mathbf{u}}} J_r' = \frac{\partial}{\partial\vec{\lambda}_i}\bigg(R_x(\lambda)\big(J_r\big)R_x^{-1}(\lambda)\bigg)=\approx 0
\end{equation}
Similarly for the inner ring without the rotor body's contribution:
\begin{equation}
R_x(\lambda)\big(J_{ir}\big)R_x^{-1}(\lambda)=J_{ir}'\rightarrow \frac{\partial}{\partial\vec{\mathbf{u}}} J_{ir}' = \frac{\partial}{\partial\vec{\lambda}_i}J_{ir}'\approx 0
