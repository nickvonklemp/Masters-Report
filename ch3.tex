...
\end{equation}
\end{subequations}
From induction factors defined Eq:\ref{eq:induction-factors}, the velocity components can be written as functions of free stream velocity $v_\infty$.
\begin{subequations}
\begin{equation}
v_b=(1+a)v_\infty
\end{equation}
...
\end{equation}
\end{subequations}
A consequence of the tangential fluid flow is that there exists an angular moment flow rate across the propeller plane. This results in a torque response to the rotational motion about the propeller's axis of rotation, analogous to Eq:\ref{eq:prop-mass}.
\begin{equation}
\vec{Q}=\rho\pi R_b^3 (v_\theta-v_\infty) v_b 
\end{equation}
Blade-element theory analyses incremental aerofoil sections of width $dr$ of the propeller profile (Fig:\ref{fig:bem-profile}) at some radius $r$. Net local fluid velocity across a single elemental aerofoil profile $\vec{U}$ is calculated as:
\begin{equation}
...
\end{equation}
\end{subequations}
Typically a power term, Eq:\ref{eq:element-power}, is given in lieu of torque or drag terms, Eq:\ref{eq:element-torque} or Eq:\ref{eq:element-drag}. Then calculating forces and torques as per momentum theory for each element, in terms of axial and tangential induction factors:
\begin{subequations}
\begin{equation}
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=0.9\textwidth]{graphs/coeffs-plot}
\vspace{-15pt}
...
\end{equation}
Where $\vec{v}_b$ is the body's transnational velocity and $\vec{\omega}_b$ is the body's angular velocity, both transformed to the propeller's frame, $\in\mathcal{F}^{M_i}$. Additionally $\hat{n}(\lambda_i,\alpha_i)$ is a unit vector normal to the propeller's rotational plane dependent on the propellers orientation relative to the body velocity. Then $J$ is calculated as in Eq:\ref{eq:advance}.
\begin{figure}[htbp]
\begin{subfigure}{0.5\textwidth}
\centering
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=0.9\textwidth]{figs/prop-coning}
\caption{Propeller coning}
\label{fig:prop-coning}
\end{figure}
\par
Both aerodynamic effects can be quantified numerically. Their derivation and resultant equations are cumbersome however. In due course their effect on the produced prototype which this project investigates aren't significant enough to produce instability if neglected. The frame could potentially be affected in more adverse ways given certain flight conditions with high transient velocities\ldots
%====================================================
\subsection{Drag}
\label{subsec:dynamics.aero.drag}
%====================================================
For any solid body with some translational velocity motion within a fluid, there is a first order damping response opposing translational velocity. The net drag, $\vec{D}_net$, although locally dependent on individual component cross sections can be abstracted to a drag coefficient matrix.
\par
\begin{equation}
...
~~~\in\mathcal{F}^b
\end{equation}
The drag coefficients, $A,B~\&~C$, are determined by the frames cross-sectional areas in the direction of each axis. Without access to wind tunnel testing, the drag coefficients are problematic to determine empirically. As such the drag effects are relegated to the lumped disturbance \& uncertainty term, Section:\ref{subsubsec:control.attitude.nonlinear.backstep}. Analogous opposition effects to angular rotation rates exist but, for the intents and purposes of most practical flight envelopes, can be disregarded.
\newpage
%====================================================
\section{Consolidated Model}
\label{sec:dynamics.model}%====================================================
Reiterating the different aspects detailed above and consolidating the state equations from Eq:\ref{eq:states.a}-\ref{eq:states.d}. Then lifting the attitude states to $\mathbb{R}^4$ space with the use of quaternions. Also introducing the non-linear inertial \& gyroscopic responses to induced perturbations, $\tau_\lambda$ and $\tau_\alpha$ from Eq:\ref{eq:torque-induced-inner} \& Eq:\ref{eq:torque-induced-middle} respectively, with non-linear inertial matrix terms $\mathbb{I}_b(u)$. Finally replacing net \emph{virtual} plant inputs\footnote{Exact actuator relationships are explored in Section:\ref{sec:control.inputs}}, $\mu \vec{\tau}$ and $\mu \vec{F}$, with higher fidelity thrust models; produces the following set of state differentials used for control plant development \ldots
\\
\begin{subequations}
\begin{equation}
\dot{\mathcal{E}}=Q_b\otimes\vec{v}\otimes Q_b^*~~~~\in\mathcal{F}^I
\end{equation}
\vspace{-10pt}
\begin{equation}
\dot{\vec{v}}=m^{-1}\big(-\vec{\omega}_b\times m\vec{v}+Q_b^*\otimes m\vec{G}\otimes Q_b-\vec{D}_{net}(\vec{v})+\mu\vec{F}\big)~~~~\in\mathcal{F}^b
\end{equation}
\vspace{-10pt}
\begin{equation}
...
\vspace{-10pt}
\begin{equation}
\dot{\vec{\omega}}_b=\mathbb{I}_b(u)^{-1}\big(-\vec{\omega}_b \times \mathbb{I}_b(u)\vec{\omega}_b+\vec{\tau}_Q(u)+\vec{\tau}_g(u)+\sum \vec{Q}(\Omega,\lambda,\alpha)+\mu\vec{\tau}\big)~~~~\in\mathcal{F}^b
\end{equation}
\end{subequations}
With net thrust and torque plant inputs (\emph{individual motor number subscripts, $i\in[1:4]$, are implied}):
\begin{subequations}
\begin{equation}
\mu\vec{F}=\sum \vec{T}(\Omega,\lambda,\alpha)
\end{equation}
\vspace{-10pt}
\begin{equation}
\mu\vec{\tau}=\sum \vec{l}\times\vec{T}(\Omega,\lambda,\alpha)
\end{equation}
\end{subequations}
The thrust $T(\Omega)$ is a function of the propellers rotational velocity but $\vec{T}(\Omega,\lambda,\alpha)$ is a 3 dimensional thrust vector redirected in the inverse of Eq:\ref{eq:motor-module-rotation.a} in the body frame $\mathcal{F}^b$. Equivalently $Q(\Omega)$ is the aerodynamic torque term in $\mathcal{F}^{M_i}$ about the motor's rotor axis and $\vec{Q}(\Omega,\lambda,\alpha)$ is the vector counterpart in $\mathcal{F}^b$. Both thrust and aerodynamic propeller torque terms are calculated from their respective coefficients:
\begin{subequations}
\begin{equation}
...
\end{equation}
\end{subequations}
Inertial torque responses (\emph{in feedback\footnote{Those response terms are used later as higher order inputs rather than terms to be compensated for.} configuration here}) from Eq:\ref{eq:torque-response};
\begin{equation}
\tau_Q(u)=\sum_{i=1}^4 -Q_{M_i}\otimes \tau_{\lambda_i}(u)\otimes Q_{M_i}^*-Q_{M_i'}\otimes \tau_{\alpha_i}(u) \otimes Q_{M_i'}^*~~~\in\mathcal{F}^b
\end{equation}
And the gravitational torque arm from Eq:\ref{eq:grav-torque}:
\begin{equation}
\vec{\tau}_g(u)=\Delta C.G \times\vec{G}_b
\end{equation}
Lastly, the body's net inertial tensor, taken from Eq:\ref{eq:body-net} is given as:
\begin{equation}
\underset{u\in\mathbb{U}}{\mathbb{I}_b(u)}=\mathbb{I}_{body}+\sum_{i=1}^{4} \mathbb{M}_{inner}+\sum_{i=1}^{4} \mathbb{M}_{middle}
\end{equation}
It is possible to bundle both attitude states (either euler angles $\eta$ or quaternions $Q_b$) together with the linear translational position $\mathcal{E}$ into a single state vector $\mathbf{X}$. Which is then has its own control law, potentially exploiting the cross-product coupling terms between angular and linear displacements.