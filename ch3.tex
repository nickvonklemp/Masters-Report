...
\end{equation}
Where the time derivative of that module's moving center of gravity, $C.M_\text{p}$ relative to the origin $\vec{\mathbf{O}}_b$, is:
\begin{equation}
\frac{d}{dt}\Delta L_i=-\frac{d}{dt}\dot{C.M}_p'
\end{equation}
\vspace{-12pt}
\begin{equation}
=-\frac{1}{m_\text{p}}\Big(m_\text{n}R_z\big([\dot{\vec{\alpha}}_i]_\times R_y(\alpha)R_x(\lambda)C.M_\text{n}+R_y(\alpha)[\dot{\vec{\lambda}}_i]_\times C.M_\text{n}\big)+m_\text{m}R_z[\dot{\vec{\alpha}}_i]_\times C.M_\text{m}\Big)
\end{equation}
\end{subequations}
...
\end{equation}
\end{subequations}
The Lagrangian for the energy of a single motor module, about the origin $\vec{\mathbf{O}}_b$, $\mathcal{L}_{p_i}$ can then be constructed. This time, including the gravitational potential energy component:
\begin{multline}
\mathcal{L}_{p_i}=\frac{1}{2}\vec{\omega}_{r/I}^T\big(J_r'''\big)\vec{\omega}_{r/I}+\frac{1}{2}\vec{\omega}_{M_i/I}^T\big(J_{ir}'''\big)\vec{\omega}_{M_i/I}+\frac{1}{2}\vec{\omega}_{M_i'/I}^T\big(J_m''\big)\vec{\omega}_{M_i'/I}
\\
+\vec{\omega}_{b/I}^T\big(m_p\big)\vec{\omega}_{b/I}-m_b\big(R_I^b(\eta)\vec{\mathcal{E}}+\Delta L_i\big)\cdot\vec{G}_b
\end{multline}
Where the term $m_b\big(R_I^b(\eta)\vec{\mathcal{E}}+\Delta L_i\big)\cdot\vec{G}_b$ is the vector analogue of gravitational potential energy $m\vec{g}h$ with $R_I^b(\eta)\vec{\mathcal{E}}$ being the inertial relative X-Y-Z position in the body frame $\mathcal{F}^b$ relative to the body origin $\vec{\mathbf{O}}_b$. Expanding $\mathcal{L}_{p_i}$ with terms defined previously:
\begin{multline}
...
\\
\Big[\dot{\vec{\alpha}}_i'+\vec{\omega}_b\Big]^T\big(J_m''\big)\Big[\dot{\vec{\alpha}}_i'+\vec{\omega}_b\Big]+\vec{\omega}_{b}^T\big(m_p\Big(\big(\Delta L_i \cdot \Delta L_i\big)\mathbb{I}_{3\times 3}-\Delta L_i \otimes \Delta L_i\Big)\big)\vec{\omega}_{b}-m_b\big(R_I^b(\eta)\vec{\mathcal{E}}+\Delta L_i\big)\cdot\vec{G}_b
\end{multline}
