...
\par
Drawing again from Lagrangian theory\footnote{The generalized linear kinetic energy for each module is an extension of that in Eq:\ref{eq:rigid-frame.a} and is independent of any of the actuator positions.} and considering only the rotational kinetic energy for the inner ring assembly $\mathcal{F}^{M_i}$, there is no permissible transnational motions between each body and as such there is no linear kinetic energy contribution. The Lagrangian for the inner ring is formed, with concern on the effect $\lambda_i$ has on the system:
\begin{equation}
...
\end{equation}
\end{subequations}
And both $\mathbb{I}_{p}$ and $\mathbb{I}_{\lambda}$ being taken W.R.T to their centre(s) of rotation. Recalling the Euler-Lagrange formulation from Eq:\ref{eq:euler-lagrange} with generalized co-ordinates $\mathbf{u}(t)$ of $\mathcal{F}^{M_i}$ relative to $\mathcal{F}^b$\footnote{Relative to the body frame and not the inertial frame because Eq:\ref{eq:states.d} accounts for the inertial response of the entire body frame. Here the induced relative responses are of concern}.
\begin{equation}
...
\end{equation}
Where $\vec{\omega}_{M_i/b}$ is the net angular velocity of the inner ring frame with respect to the body frame:
\begin{equation}
...
\end{equation}
The net torque from a $\lambda_i$ rotation, induced in the motor module frame $\mathcal{F}^{M_i}$, is:
...
\end{equation}
\end{subequations}
Similarly for the middle ring response with inertia a function of the $\lambda$ angle, $\mathbb{I}_{\alpha}(\lambda_i)$ from Eq:\ref{eq:inertia.middle.c} with generalized co-ordinates $\mathbf{v}(t)$ of $\mathcal{F}^{M_i'}$ relative to $\mathcal{F}^b$:
\begin{subequations}
\begin{equation}
...
\end{equation}
\end{subequations}
Each of the torques, $\vec{\tau_}\lambda$ \& $\vec{\tau}(\lambda_i)_\alpha$, occur in intermediary frames associated with the inner and middle ring assemblies. As such, their negative responses affect Eq:\ref{eq:states.d} or Eq:\ref{eq:rigid-frame.b}, 

they need to be transformed to the body frame.
\begin{equation}

\end{equation}
The above responses are pertinent to simulation and plant dependent compensation. The simulation environment is structured such that the torques are produced as responses from Newtonian movement at every step interval. In due course it would be more efficient (and less stiff) to exploit an implicit Euler\cite{.} coordinate system in substitution for the cartesian response equations developed above. However this was not implemented in Chapter:\ref{ch:simulation}.
%====================================================
