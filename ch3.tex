...
\end{equation}
\end{subequations}
In general, for some transformed inertia $J'$, transformed by some $R_a(\theta)$ between two reference frames, its time derivative can be given as:
\begin{equation}
...
\end{equation}
Where $\dot{\vec{\theta}}$ is the angular velocity vector between the two frames. Inserting those transformed inertial derivatives into Eq:\ref{eq:3.42b} and using Reynolds transportation theorem, Eq:\ref{eq:reynolds} for a vector's derivative in a rotating reference frame. The product rule then yields:
\begin{multline}
...
\end{equation}
Taking special care to differentiate $J_{n}'$ and $J_{m}$. The former being the inner ring's inertia transformed into the middle ring frame $\mathcal{F}^{M_i'}$ and the latter is the middle ring (only) structure's inertia in that same reference frame. 
\par
...
\end{equation}
\end{subequations}
Note that the relations of Eq:\ref{eq:middle-inertia-relation} and Eq:\ref{eq:middle-inertia-rate} to the collective body inertia $J_p$ is to simplify the subsequent equations. Each body then has its own relative angular velocity with respect to the intermediate frame $\mathcal{F}^{M_i''}$. For the rotor; $\vec{\omega}_{r/M_i''}$ is the relative angular velocity of that assembly:
...
\end{multline}
\end{subequations}
Noting that $\dot{J}_p = \dot{J}_r'+\dot{J}_{ir}'+\dot{J}_m$ and that $\dot{J}_m=0$, it follows that $\dot{J}_p =\dot{J}_n'$. Then isolating the torque response from $\Delta\alpha$, and again grouping inertial bodies with shared angular velocities together. The $\emph{inertial~rate}$,second order \emph{inertial} and first order \emph{gyroscopic} responses are then:
\begin{multline} \label{eq:torque-induced-middle}
\vec{\tau}_\alpha(\lambda_i)=\underbrace{\big(\dot{J}_p'\big)\dot{\vec{\alpha}}_i+\Big(\dot{J}_n''-R_y(\alpha)\big(\dot{J}_n'\big)R_y^{-1}(\alpha)\Big)\dot{\vec{\lambda}}_i'+\Big(\dot{J}_r''-R_y(\alpha)\big(\dot{J}_r'\big)R_y^{-1}(\alpha)\Big)\vec{\Omega}_i''}_{Inertial~rate}
\\
...
\end{figure}
<<<<<<< HEAD
The final non-trivial torque term to account for is as a result of the response which relative rotations $\lambda$ and $\alpha$ have to the net angular velocity of the entire multibody system $\vec{\omega}_b$. Such responses are an extension of the fundamental rigid 6-DOF differential equation for angular motion from Eq:\ref{eq:states.d}:
\begin{equation}
...
\end{equation}
Separating the motor module from the entire body structure first, and only considering the additional contribution $\vec{\omega}_b$ has on the net torque acting on the body, the Lagrangian  derivation for motion relative to the inertial frame then follows.
\par
The relative $\hat{Z}_{M_i''}$ rotation is what differentiates the intermediate frame $\mathcal{F}^{M_i''}$, using for calculations pertinent to Fig:\ref{fig:response-middle} and the body frame $\mathcal{F}^b$. The familiar rotor assembly, inner and middle rings inertias from Eq:\ref{eq:rotor-inertia-mid},\ref{eq:inner-inertia-mid} and \ref{eq:middle-inertia-mid} have respective counterparts aligned with $\mathcal{F}^b$:
...
\begin{subequations}
\begin{equation}
\vec{\omega}_{r/I}=\vec{\Omega}_i'''+\dot{\vec{\lambda}}_i''+\dot{\vec{\alpha}}_i'+\vec{\omega}_{b/I}
\end{equation}
\vspace{-12pt}
\begin{equation}
...
\end{equation}
\end{subequations}
For a single motor module, relative to the inertial frame with extra contributing angular velocity $\vec{\omega}_b$
=======
The final non-trivial torque term to account for is as a result of the response relative rotations have to the net angular velocity of the entire multibody system. That being the inertial and gyroscopic responses to net angular velocity $\vec{\omega}_{b/I}$ relative to the inertial frame. Such responses are an extension of the fundamental rigid 6-DOF differential equation for angular motion from Eq:\ref{eq:states.d}
\begin{subequations};
\begin{equation}
\dot{\vec{\omega}}_b=\big(J_b^{-1}\big)\Big(-\vec{\omega}_b\times\big(J_b\big)\vec{\omega}_b+\vec{\tau}_{\mu}\Big)~~~~[\text{rad.s}^{\text{-}s}],~\in\mathcal{F}^b
\end{equation}
Where $J_b$ in Eq:\ref{eq:states.d} is the entire rigid body's inertia and $\vec{\omega}_b$ is the body's angular velocity relative to the inertial frame. Again taking stock of all the bodies within a motor module and forming a Lagrangian of their motion relative to $\mathcal{F}^{I}$ but in the body frame $\mathcal{F}^{b}$. Fig:\ref{fig:response-body} shows the exploded view of all affected bodies; 


Considering only a single motor module, similar to Eq:\ref{eq:alpha-lagrange-deriv} but with added $\vec{\omega}_{b/I}$ contributions:

>>>>>>> c97c01b539c7087334a510a657cefdb1016a9393
